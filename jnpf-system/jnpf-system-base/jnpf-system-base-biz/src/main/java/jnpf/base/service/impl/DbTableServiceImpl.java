...
public class DbTableServiceImpl implements DbTableService {
    ...
    public List<DbTableModel> getList(String dbId) throws DataException {
        ...
        if (link != null) {
            ...
            if (conn != null) {
                if (dataSourceUtils.getUrl().contains(jnpf.emnus.DbType.MYSQL.getMessage())) {
                    list = DbUtil.mysqlgetList(conn, dbName);
                } else if (dataSourceUtils.getUrl().contains(jnpf.emnus.DbType.SQLSERVER.getMessage())) {
                    list = DbUtil.sqlServergetList(conn, dbName);
                } else if (dataSourceUtils.getUrl().contains(jnpf.emnus.DbType.ORACLE.getMessage())) {
